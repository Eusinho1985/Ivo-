...
set(Ivo__WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(Ivo__COI_WORKER "${Ivo__WEB_DIR}/coi-serviceworker.min.js")
set(Ivo__SHELL_TEMPLATE "${Ivo__WEB_DIR}/shell_template.html.in")
set(Ivo__INDEX_TEMPLATE "${Ivo__WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(Ivo__WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(Ivo__WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(Ivo__WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(Ivo__configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY Ivo__WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE Ivo__WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${Ivo__WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${Ivo__WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${Ivo__WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${Ivo__SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${Ivo__SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${Ivo__COI_WORKER}")
...
function(Ivo__create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY Ivo__WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${Ivo__INDEX_TEMPLATE}")
    configure_file("${Ivo__INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${Ivo__INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY Ivo__WASM_TARGET_${target}_OUTPUT_NAME)
